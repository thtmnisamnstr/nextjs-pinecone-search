...
Goal: you should be able to create a Pinecone account, copy an API key, make a few changes, and have working search.
...
You only provide index names. If those indexes do not exist yet, this package can create compatible serverless integrated indexes for you.
...
This triggers a full reindex on `next dev` and `next build`.

### 6) Add the search API route

App Router (`app/api/pinecone-search/route.ts`):
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { search, query, topK, rerankTopN } = req.body ?? {};

  if (typeof search !== "string" || typeof query !== "string") {
    return res.status(400).json({ error: "Expected body: { search, query, topK?, rerankTopN? }" });
  }

  try {
    const result = await service.search(search, query, {
      topK: typeof topK === "number" ? topK : undefined,
      rerankTopN: typeof rerankTopN === "number" ? rerankTopN : undefined
    });
...
- Auto reindex during `dev` and `build`
