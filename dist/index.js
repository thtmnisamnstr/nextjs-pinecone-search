...
function defaultResolveUrl(args) {
  ...
  const ext = path2.extname(args.relativePath);
  ...
  const prefixed = args.routePrefix ? normalizeUrlPath(path2.posix.join(args.routePrefix, routePath)) : normalizeUrlPath(routePath);
...
async function resolveModule(filePath) {
  ...
  if (ext === ".ts" || ext === ".mts" || ext === ".cts") {
    const jiti = jitiFactory(path2.dirname(filePath), { interopDefault: true });
...
async function loadPineconeSearchConfig(options) {
  ...
  const configPath = options.configPath ? path2.resolve(cwd, options.configPath) : envPath ? path2.resolve(cwd, envPath) : await findDefaultConfigPath(cwd);
  if (!configPath) {
    throw new Error(
...
async function findDefaultConfigPath(cwd) {
  for (const filename of DEFAULT_CONFIG_FILENAMES) {
    const absolutePath = path2.resolve(cwd, filename);
    try {
...
function chooseRelativePath(filePath, baseDirs, cwd) {
  const normalizedFilePath = toPosixPath(path3.resolve(filePath));
  let best;
  for (const dir of baseDirs) {
    const absBase = toPosixPath(path3.resolve(cwd, dir));
  ...
  return toPosixPath(path3.relative(cwd, filePath));
}
...
async function discoverSearchDocuments(search, cwd) {
  ...
  for (const sourceDef of search.sources) {
    ...
    const toProcess = uniqueFiles.map((absoluteFilePath) => path3.resolve(absoluteFilePath)).filter((absoluteFilePath) => !deduped.has(absoluteFilePath));
    const documents = await mapWithConcurrency(toProcess, FILE_READ_CONCURRENCY, async (absoluteFilePath) => {
      const source = await fs2.readFile(absoluteFilePath, "utf8");
...
var PineconeSearchService = class {
  ...
  constructor(options) {
    ...
    this.cwd = options.cwd ? path4.resolve(options.cwd) : process.cwd();
  ...
  async reindexAll() {
    ...
    for (const [searchName, searchDefinition] of Object.entries(this.config.searches)) {
      ...
      for (const document of documents) {
        ...
        const sourcePath = toPosixPath(path4.relative(this.cwd, document.filePath));
...
function getReindexMarkerPath(cwd, command) {
  return path5.join(cwd, ".next", "cache", `nextjs-pinecone-search-${command}.run-key`);
}
...
async function hasReindexedInProcessGroup(cwd, command, runKey) {
  ...
  await fs3.mkdir(path5.dirname(markerPath), { recursive: true });
  try {
    const existingKey = (await fs3.readFile(markerPath, "utf8")).trim();
    if (existingKey === runKey) {
      return true;
    }
  } catch {
  }
  await fs3.writeFile(markerPath, runKey);
  return false;
}
